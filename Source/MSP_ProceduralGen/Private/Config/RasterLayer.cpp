...
FVector2D URasterLayer::GetCoordinate0AsWorldVector2D() const
{
...
FVector2D URasterLayer::GetCoordinate1AsWorldVector2D() const
{
	const FVector2D& worldOffset = GetWorldOffset();
	return FVector2D(
...
FVector2D URasterLayer::GetWorldOffset() const
{
	return GetOffset() * UConfig::MetersToUnrealUnits;
}

FVector2D URasterLayer::GetOffset() const
{
